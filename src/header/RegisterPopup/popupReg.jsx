...
function reducer(
    ...
    action) // событие, которое передали в dispatch
  {
  switch (action.type) { // какое событие произошло?
    case 'loginChanged': { // поменялось поле Логин
      return  { // возвращаем
        ...state, // старое состояние +
    ...
    case 'passChanged': { // поменялось поле Пароль
      return  { // возвращаем
        ...state, // старое состояние +
    ...
    case 'confirmChanged': { // поменялось поле Подтверждение пароля
      return  { // возвращаем
        ...state, // старое состояние +
...
export default function RegistrationForm({ visibilityReg, setVisibilityReg }) {
  ...
  function handleLoginInput(e) { // хендлер (обработчик) события onInput поля Логин
    dispatch({ // отправляем событие
